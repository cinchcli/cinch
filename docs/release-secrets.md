...
| `TAURI_SIGNING_PRIVATE_KEY` | `publish.yml` (build-desktop-*) | Minisign private key for signing Tauri updater payloads; clients verify against the embedded pubkey in `apps/desktop/src-tauri/tauri.conf.json`. **Must be the base64-encoded file contents** (`cat ~/.tauri/cinch.key \| base64`), NOT the raw `.key` file with its `untrusted comment:` header — Tauri 2's bundler `base64::decode`s this directly and fails on the space in the header at offset 9. | **Never rotate without forcing a manual reinstall** — clients verify with the pinned pubkey; a new key invalidates all existing installs' update path |

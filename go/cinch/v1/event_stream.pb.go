// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: cinch/v1/event_stream.proto

package cinchv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type SubscribeRequest struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SubscribeRequest) Reset() {
	*x = SubscribeRequest{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SubscribeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SubscribeRequest) ProtoMessage() {}

func (x *SubscribeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SubscribeRequest.ProtoReflect.Descriptor instead.
func (*SubscribeRequest) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{0}
}

type ClipDeletedEvent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClipId        string                 `protobuf:"bytes,1,opt,name=clip_id,json=clipId,proto3" json:"clip_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClipDeletedEvent) Reset() {
	*x = ClipDeletedEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClipDeletedEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClipDeletedEvent) ProtoMessage() {}

func (x *ClipDeletedEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClipDeletedEvent.ProtoReflect.Descriptor instead.
func (*ClipDeletedEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{1}
}

func (x *ClipDeletedEvent) GetClipId() string {
	if x != nil {
		return x.ClipId
	}
	return ""
}

type ClipPinnedEvent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ClipId        string                 `protobuf:"bytes,1,opt,name=clip_id,json=clipId,proto3" json:"clip_id,omitempty"`
	IsPinned      bool                   `protobuf:"varint,2,opt,name=is_pinned,json=isPinned,proto3" json:"is_pinned,omitempty"`
	PinNote       *string                `protobuf:"bytes,3,opt,name=pin_note,json=pinNote,proto3,oneof" json:"pin_note,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ClipPinnedEvent) Reset() {
	*x = ClipPinnedEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ClipPinnedEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ClipPinnedEvent) ProtoMessage() {}

func (x *ClipPinnedEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ClipPinnedEvent.ProtoReflect.Descriptor instead.
func (*ClipPinnedEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{2}
}

func (x *ClipPinnedEvent) GetClipId() string {
	if x != nil {
		return x.ClipId
	}
	return ""
}

func (x *ClipPinnedEvent) GetIsPinned() bool {
	if x != nil {
		return x.IsPinned
	}
	return false
}

func (x *ClipPinnedEvent) GetPinNote() string {
	if x != nil && x.PinNote != nil {
		return *x.PinNote
	}
	return ""
}

type ServerEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Event:
	//
	//	*ServerEvent_NewClip
	//	*ServerEvent_Revoked
	//	*ServerEvent_TokenRotated
	//	*ServerEvent_KeyExchange
	//	*ServerEvent_ClipDeleted
	//	*ServerEvent_ClipPinned
	Event         isServerEvent_Event `protobuf_oneof:"event"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ServerEvent) Reset() {
	*x = ServerEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ServerEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServerEvent) ProtoMessage() {}

func (x *ServerEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServerEvent.ProtoReflect.Descriptor instead.
func (*ServerEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{3}
}

func (x *ServerEvent) GetEvent() isServerEvent_Event {
	if x != nil {
		return x.Event
	}
	return nil
}

func (x *ServerEvent) GetNewClip() *NewClipEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_NewClip); ok {
			return x.NewClip
		}
	}
	return nil
}

func (x *ServerEvent) GetRevoked() *RevokedEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_Revoked); ok {
			return x.Revoked
		}
	}
	return nil
}

func (x *ServerEvent) GetTokenRotated() *TokenRotatedEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_TokenRotated); ok {
			return x.TokenRotated
		}
	}
	return nil
}

func (x *ServerEvent) GetKeyExchange() *KeyExchangeEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_KeyExchange); ok {
			return x.KeyExchange
		}
	}
	return nil
}

func (x *ServerEvent) GetClipDeleted() *ClipDeletedEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_ClipDeleted); ok {
			return x.ClipDeleted
		}
	}
	return nil
}

func (x *ServerEvent) GetClipPinned() *ClipPinnedEvent {
	if x != nil {
		if x, ok := x.Event.(*ServerEvent_ClipPinned); ok {
			return x.ClipPinned
		}
	}
	return nil
}

type isServerEvent_Event interface {
	isServerEvent_Event()
}

type ServerEvent_NewClip struct {
	NewClip *NewClipEvent `protobuf:"bytes,1,opt,name=new_clip,json=newClip,proto3,oneof"`
}

type ServerEvent_Revoked struct {
	Revoked *RevokedEvent `protobuf:"bytes,3,opt,name=revoked,proto3,oneof"`
}

type ServerEvent_TokenRotated struct {
	TokenRotated *TokenRotatedEvent `protobuf:"bytes,4,opt,name=token_rotated,json=tokenRotated,proto3,oneof"`
}

type ServerEvent_KeyExchange struct {
	KeyExchange *KeyExchangeEvent `protobuf:"bytes,5,opt,name=key_exchange,json=keyExchange,proto3,oneof"`
}

type ServerEvent_ClipDeleted struct {
	ClipDeleted *ClipDeletedEvent `protobuf:"bytes,6,opt,name=clip_deleted,json=clipDeleted,proto3,oneof"`
}

type ServerEvent_ClipPinned struct {
	ClipPinned *ClipPinnedEvent `protobuf:"bytes,7,opt,name=clip_pinned,json=clipPinned,proto3,oneof"`
}

func (*ServerEvent_NewClip) isServerEvent_Event() {}

func (*ServerEvent_Revoked) isServerEvent_Event() {}

func (*ServerEvent_TokenRotated) isServerEvent_Event() {}

func (*ServerEvent_KeyExchange) isServerEvent_Event() {}

func (*ServerEvent_ClipDeleted) isServerEvent_Event() {}

func (*ServerEvent_ClipPinned) isServerEvent_Event() {}

type NewClipEvent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Clip          *Clip                  `protobuf:"bytes,1,opt,name=clip,proto3" json:"clip,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NewClipEvent) Reset() {
	*x = NewClipEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NewClipEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NewClipEvent) ProtoMessage() {}

func (x *NewClipEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NewClipEvent.ProtoReflect.Descriptor instead.
func (*NewClipEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{4}
}

func (x *NewClipEvent) GetClip() *Clip {
	if x != nil {
		return x.Clip
	}
	return nil
}

type RevokedEvent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Reason        string                 `protobuf:"bytes,1,opt,name=reason,proto3" json:"reason,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RevokedEvent) Reset() {
	*x = RevokedEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RevokedEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RevokedEvent) ProtoMessage() {}

func (x *RevokedEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RevokedEvent.ProtoReflect.Descriptor instead.
func (*RevokedEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{5}
}

func (x *RevokedEvent) GetReason() string {
	if x != nil {
		return x.Reason
	}
	return ""
}

type TokenRotatedEvent struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Token         string                 `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	DeviceId      string                 `protobuf:"bytes,2,opt,name=device_id,json=deviceId,proto3" json:"device_id,omitempty"`
	Hostname      string                 `protobuf:"bytes,3,opt,name=hostname,proto3" json:"hostname,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TokenRotatedEvent) Reset() {
	*x = TokenRotatedEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TokenRotatedEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TokenRotatedEvent) ProtoMessage() {}

func (x *TokenRotatedEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TokenRotatedEvent.ProtoReflect.Descriptor instead.
func (*TokenRotatedEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{6}
}

func (x *TokenRotatedEvent) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

func (x *TokenRotatedEvent) GetDeviceId() string {
	if x != nil {
		return x.DeviceId
	}
	return ""
}

func (x *TokenRotatedEvent) GetHostname() string {
	if x != nil {
		return x.Hostname
	}
	return ""
}

type KeyExchangeEvent struct {
	state                protoimpl.MessageState `protogen:"open.v1"`
	DeviceId             string                 `protobuf:"bytes,1,opt,name=device_id,json=deviceId,proto3" json:"device_id,omitempty"`
	Hostname             string                 `protobuf:"bytes,2,opt,name=hostname,proto3" json:"hostname,omitempty"`
	DeviceKeyFingerprint string                 `protobuf:"bytes,3,opt,name=device_key_fingerprint,json=deviceKeyFingerprint,proto3" json:"device_key_fingerprint,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *KeyExchangeEvent) Reset() {
	*x = KeyExchangeEvent{}
	mi := &file_cinch_v1_event_stream_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *KeyExchangeEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*KeyExchangeEvent) ProtoMessage() {}

func (x *KeyExchangeEvent) ProtoReflect() protoreflect.Message {
	mi := &file_cinch_v1_event_stream_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use KeyExchangeEvent.ProtoReflect.Descriptor instead.
func (*KeyExchangeEvent) Descriptor() ([]byte, []int) {
	return file_cinch_v1_event_stream_proto_rawDescGZIP(), []int{7}
}

func (x *KeyExchangeEvent) GetDeviceId() string {
	if x != nil {
		return x.DeviceId
	}
	return ""
}

func (x *KeyExchangeEvent) GetHostname() string {
	if x != nil {
		return x.Hostname
	}
	return ""
}

func (x *KeyExchangeEvent) GetDeviceKeyFingerprint() string {
	if x != nil {
		return x.DeviceKeyFingerprint
	}
	return ""
}

var File_cinch_v1_event_stream_proto protoreflect.FileDescriptor

const file_cinch_v1_event_stream_proto_rawDesc = "" +
	"\n" +
	"\x1bcinch/v1/event_stream.proto\x12\bcinch.v1\x1a\x14cinch/v1/clips.proto\"\x12\n" +
	"\x10SubscribeRequest\"+\n" +
	"\x10ClipDeletedEvent\x12\x17\n" +
	"\aclip_id\x18\x01 \x01(\tR\x06clipId\"t\n" +
	"\x0fClipPinnedEvent\x12\x17\n" +
	"\aclip_id\x18\x01 \x01(\tR\x06clipId\x12\x1b\n" +
	"\tis_pinned\x18\x02 \x01(\bR\bisPinned\x12\x1e\n" +
	"\bpin_note\x18\x03 \x01(\tH\x00R\apinNote\x88\x01\x01B\v\n" +
	"\t_pin_note\"\x99\x03\n" +
	"\vServerEvent\x123\n" +
	"\bnew_clip\x18\x01 \x01(\v2\x16.cinch.v1.NewClipEventH\x00R\anewClip\x122\n" +
	"\arevoked\x18\x03 \x01(\v2\x16.cinch.v1.RevokedEventH\x00R\arevoked\x12B\n" +
	"\rtoken_rotated\x18\x04 \x01(\v2\x1b.cinch.v1.TokenRotatedEventH\x00R\ftokenRotated\x12?\n" +
	"\fkey_exchange\x18\x05 \x01(\v2\x1a.cinch.v1.KeyExchangeEventH\x00R\vkeyExchange\x12?\n" +
	"\fclip_deleted\x18\x06 \x01(\v2\x1a.cinch.v1.ClipDeletedEventH\x00R\vclipDeleted\x12<\n" +
	"\vclip_pinned\x18\a \x01(\v2\x19.cinch.v1.ClipPinnedEventH\x00R\n" +
	"clipPinnedB\a\n" +
	"\x05eventJ\x04\b\x02\x10\x03R\x0esend_clipboard\"2\n" +
	"\fNewClipEvent\x12\"\n" +
	"\x04clip\x18\x01 \x01(\v2\x0e.cinch.v1.ClipR\x04clip\"&\n" +
	"\fRevokedEvent\x12\x16\n" +
	"\x06reason\x18\x01 \x01(\tR\x06reason\"b\n" +
	"\x11TokenRotatedEvent\x12\x14\n" +
	"\x05token\x18\x01 \x01(\tR\x05token\x12\x1b\n" +
	"\tdevice_id\x18\x02 \x01(\tR\bdeviceId\x12\x1a\n" +
	"\bhostname\x18\x03 \x01(\tR\bhostname\"\x81\x01\n" +
	"\x10KeyExchangeEvent\x12\x1b\n" +
	"\tdevice_id\x18\x01 \x01(\tR\bdeviceId\x12\x1a\n" +
	"\bhostname\x18\x02 \x01(\tR\bhostname\x124\n" +
	"\x16device_key_fingerprint\x18\x03 \x01(\tR\x14deviceKeyFingerprint2V\n" +
	"\x12EventStreamService\x12@\n" +
	"\tSubscribe\x12\x1a.cinch.v1.SubscribeRequest\x1a\x15.cinch.v1.ServerEvent0\x01B/Z-github.com/cinchcli/cinch/go/cinch/v1;cinchv1b\x06proto3"

var (
	file_cinch_v1_event_stream_proto_rawDescOnce sync.Once
	file_cinch_v1_event_stream_proto_rawDescData []byte
)

func file_cinch_v1_event_stream_proto_rawDescGZIP() []byte {
	file_cinch_v1_event_stream_proto_rawDescOnce.Do(func() {
		file_cinch_v1_event_stream_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_cinch_v1_event_stream_proto_rawDesc), len(file_cinch_v1_event_stream_proto_rawDesc)))
	})
	return file_cinch_v1_event_stream_proto_rawDescData
}

var file_cinch_v1_event_stream_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_cinch_v1_event_stream_proto_goTypes = []any{
	(*SubscribeRequest)(nil),  // 0: cinch.v1.SubscribeRequest
	(*ClipDeletedEvent)(nil),  // 1: cinch.v1.ClipDeletedEvent
	(*ClipPinnedEvent)(nil),   // 2: cinch.v1.ClipPinnedEvent
	(*ServerEvent)(nil),       // 3: cinch.v1.ServerEvent
	(*NewClipEvent)(nil),      // 4: cinch.v1.NewClipEvent
	(*RevokedEvent)(nil),      // 5: cinch.v1.RevokedEvent
	(*TokenRotatedEvent)(nil), // 6: cinch.v1.TokenRotatedEvent
	(*KeyExchangeEvent)(nil),  // 7: cinch.v1.KeyExchangeEvent
	(*Clip)(nil),              // 8: cinch.v1.Clip
}
var file_cinch_v1_event_stream_proto_depIdxs = []int32{
	4, // 0: cinch.v1.ServerEvent.new_clip:type_name -> cinch.v1.NewClipEvent
	5, // 1: cinch.v1.ServerEvent.revoked:type_name -> cinch.v1.RevokedEvent
	6, // 2: cinch.v1.ServerEvent.token_rotated:type_name -> cinch.v1.TokenRotatedEvent
	7, // 3: cinch.v1.ServerEvent.key_exchange:type_name -> cinch.v1.KeyExchangeEvent
	1, // 4: cinch.v1.ServerEvent.clip_deleted:type_name -> cinch.v1.ClipDeletedEvent
	2, // 5: cinch.v1.ServerEvent.clip_pinned:type_name -> cinch.v1.ClipPinnedEvent
	8, // 6: cinch.v1.NewClipEvent.clip:type_name -> cinch.v1.Clip
	0, // 7: cinch.v1.EventStreamService.Subscribe:input_type -> cinch.v1.SubscribeRequest
	3, // 8: cinch.v1.EventStreamService.Subscribe:output_type -> cinch.v1.ServerEvent
	8, // [8:9] is the sub-list for method output_type
	7, // [7:8] is the sub-list for method input_type
	7, // [7:7] is the sub-list for extension type_name
	7, // [7:7] is the sub-list for extension extendee
	0, // [0:7] is the sub-list for field type_name
}

func init() { file_cinch_v1_event_stream_proto_init() }
func file_cinch_v1_event_stream_proto_init() {
	if File_cinch_v1_event_stream_proto != nil {
		return
	}
	file_cinch_v1_clips_proto_init()
	file_cinch_v1_event_stream_proto_msgTypes[2].OneofWrappers = []any{}
	file_cinch_v1_event_stream_proto_msgTypes[3].OneofWrappers = []any{
		(*ServerEvent_NewClip)(nil),
		(*ServerEvent_Revoked)(nil),
		(*ServerEvent_TokenRotated)(nil),
		(*ServerEvent_KeyExchange)(nil),
		(*ServerEvent_ClipDeleted)(nil),
		(*ServerEvent_ClipPinned)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_cinch_v1_event_stream_proto_rawDesc), len(file_cinch_v1_event_stream_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_cinch_v1_event_stream_proto_goTypes,
		DependencyIndexes: file_cinch_v1_event_stream_proto_depIdxs,
		MessageInfos:      file_cinch_v1_event_stream_proto_msgTypes,
	}.Build()
	File_cinch_v1_event_stream_proto = out.File
	file_cinch_v1_event_stream_proto_goTypes = nil
	file_cinch_v1_event_stream_proto_depIdxs = nil
}

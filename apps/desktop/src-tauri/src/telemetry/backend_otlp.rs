...
//!
//! Mirrors the CLI backend (`crates/cli/src/telemetry/backend_otlp.rs`); the only
//! desktop differences are the surface ("desktop"), the `cinch-desktop` user
//! agent, and the `buffer_len` accessor used by the threshold-triggered flush.
...
/// The POST body. `app` names the producing surface ("desktop") so the relay
/// records these under their own service.name. `anon_id` is the raw
/// client-generated UUID; the relay HMACs it before forwarding, so sending it raw
/// here is intentional and not identity.
#[derive(Serialize, Debug, Clone, PartialEq, Eq)]
struct OtlpBatch {
    app: String,
    anon_id: String,
...
impl OtlpBackend {
    ...
    /// Number of buffered events. Used by the threshold-triggered flush in
    /// `mod.rs` (FLUSH_THRESHOLD). A poisoned lock reports 0 so the caller never
    /// panics on a telemetry path.
    ...
    pub async fn flush(&self) {
        ...
        let user_agent = format!("cinch-desktop/{}", APP_VERSION);
        let _ = self
            .http
            .post(&url)
...
mod tests {
    ...
    fn coerces_property_types_to_strings() {
        let event = Event::new("desktop.app.opened")
            .with("is_configured", true)
            .with("failed", false)
            .with("command", "open")
        ...
        assert_eq!(attr(attrs, "is_configured"), Some("true"));
        assert_eq!(attr(attrs, "failed"), Some("false"));
        assert_eq!(attr(attrs, "command"), Some("open"));
    ...
    fn does_not_carry_clipboard_content() {
        ...
        let batch = build_batch(SAMPLE_ID, vec![Event::new("desktop.app.opened")]);
        let attrs = &batch.events[0].attrs;
    ...
    fn preserves_anon_id_and_event_name() {
        ...
        assert_eq!(batch.events[0].name, "desktop.app.opened");
    }

    #[test]
    fn serialized_payload_matches_wire_contract_shape() {
        let event = Event::new("desktop.app.opened").with("is_configured", true);
        let batch = build_batch(SAMPLE_ID, vec![event]);
        ...
        assert_eq!(json["app"], "desktop");
        assert_eq!(json["anon_id"], SAMPLE_ID);
        let first = &json["events"][0];
        assert_eq!(first["name"], "desktop.app.opened");
    ...
    fn empty_relay_base_flush_is_noop() {
        ...
        assert_eq!(backend.buffer_len(), 1);
        ...
        assert_eq!(backend.buffer_len(), 0);
        rt.block_on(backend.flush());
    }
}
